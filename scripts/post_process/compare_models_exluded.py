...
def _drop_excluded_keypoint_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Strip tail keypoint columns from the label dataframe before it's
    passed into benchmark.py's helpers, so load_all/print_per_keypoint_table
    (which derive their keypoint list from df's columns) never see them.

    Note: this also removes tail points from whatever bbox-diagonal scale
    computation benchmark.py performs internally, since it operates on this
    same df. If you want the scale computed from the full body (including
    tail) while still excluding tail from the error metrics, that needs a
    change inside benchmark.py itself instead of here.
    """
    drop_cols = [
        c for c in df.columns
        if c[:-2] in EXCLUDED_KEYPOINTS and (c.endswith("_x") or c.endswith("_y"))
    ]
...
def run_t_tests(old_kp: dict[str, list[float]], new_kp: dict[str, list[float]],
    ...
    all_kps = sorted((set(old_kp) | set(new_kp)) - EXCLUDED_KEYPOINTS)
...
def main() -> None:
    ...
    df = _drop_excluded_keypoint_columns(df)
